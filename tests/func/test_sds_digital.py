...
def test_returns(browser, test_client, db):
    ...
    path = os.getcwd() + "/tests/files/sds_digital_test1.csv"
    ...
    assert pending_statement.text == 'sds_digital_test1.csv'
    ...
    browser.find_element_by_id('QZDZE1905x03').click()
    browser.find_element_by_id('new_value').click()
    browser.find_element_by_id('QZDZE1905003').click()
    ...
    rows = table.find_elements_by_tag_name('tr')
    assert rows[1].find_elements_by_tag_name('td')[0].text == 'sds_digital_test1.csv'
    browser.find_element_by_id('view1').click()
    
    """ User goes to imported income statement detail to view summa to view summary."""
    time.sleep(1)
    assert browser.find_element_by_id('number_of_records').text == '13'
